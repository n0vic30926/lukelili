...
Status: data-quality foundation stabilized; mocked report tests cover success, partial data failure, skipped news, example portfolio mode, macro freshness, rule-based macro interpretation, ETF risk metadata, user-confirmation action tracking, and confirmed-action outcome status.
...
- expand historical review beyond outcome status into outcome quality scoring and checklist evidence;
...
- historical review aggregates confirmed, pending, and rejected actions plus confirmed-action outcome status without printing private details.
