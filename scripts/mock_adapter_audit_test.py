...
class FakeSchemaAkShare:
    @staticmethod
    def macro_china_cpi_monthly():
        return [{"月份": "2026-03", "今值": 0.7}]

    @staticmethod
    def macro_china_pmi_yearly():
        return [{"报告期": "2026-03", "指数": 50.8}]
...
def run_macro_schema_audit_test():
    settings = {
        "fed_policy_calendar": {
            "source": "FOMC fixture",
            "source_tier": "official",
    ...
    audit = audit_settings(settings, ak_module=FakeSchemaAkShare, check_output_schema=True)
    report = "\n".join(format_audit(audit))
    _assert_contains(report, "宏观 adapter: 中国CPI | status=ok | available=macro_china_cpi_monthly | schema=ok")
    _assert_contains(report, "宏观 adapter: 中国PMI | status=schema_issue")
    _assert_contains(report, "schema_issues=missing_date_column,missing_value_column")


def main():
    run_adapter_audit_test()
    run_macro_schema_audit_test()
