...
- `scripts/mock_decision_tracker_test.py`：离线测试用户确认动作与确认后结果复盘字段，不触碰真实持仓。
- `scripts/report_index.py`：查看本地报告归档索引。
- `scripts/review_history.py`：聚合报告索引、决策记录、用户确认动作和确认后结果复盘，做纪律复盘摘要与策略评分卡。
...
复盘只输出聚合信息、策略级检查、用户确认状态和确认后结果状态，不打印成本、份额、资产代码、金额、理由或结果说明全文。
